"""Auto-generated Add IAM Wildcard Policy Detector for Overly Permissive Roles.

Implement a new detector in the detectors module that identifies IAM policies with
    wildcard actions (*) or wildcard resources (*) across AWS, Azure, and GCP. This
    addresses a critical cloud security misconfiguration where excessive permissions
    increase blast radius. The detector should include severity scoring, remediation
...
__all__ = ["IamWildcardPolicyDetectorOverlyPermissiveRoles"]

logger = logging.getLogger(__name__)


class IamWildcardPolicyDetectorOverlyPermissiveRoles:
    """Scanner for Add IAM Wildcard Policy Detector for Overly Permissive Roles.
    ...
    name: str = "Add IAM Wildcard Policy Detector for Overly Permissive Roles"
    ...
    def _check_encryption(self, config: dict[str, Any]) -> list[Finding]:
        ...
        if not config.get("encryption_enabled", False):
            findings.append(
                Finding(
                    id="FEATIAMWILDCARDDETECTOR-001",
    ...
    def _check_public_access(self, config: dict[str, Any]) -> list[Finding]:
        ...
        if config.get("publicly_accessible", False):
            findings.append(
                Finding(
                    id="FEATIAMWILDCARDDETECTOR-002",
    ...
    def _check_logging(self, config: dict[str, Any]) -> list[Finding]:
        ...
        if not config.get("logging_enabled", False):
            findings.append(
                Finding(
                    id="FEATIAMWILDCARDDETECTOR-003",
